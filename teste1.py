...
# lista = [0] * 1000000000 # maneira mais rápida de criar lista de zeros
# end_time = time.time()
# elapsed_time = end_time - start_time
# print(f'A criação de lista de 1 bilhão de elementos levou: {elapsed_time}')

# start_time = time.time()
# ndarray = numpy.zeros(1000000000) # maneira mais rápida de criar lista de zeros com o numpy
# end_time = time.time()
# elapsed_time = end_time - start_time
...
# matriz = rng.random([4, 4])
# m_coluna = numpy.sort(matriz, axis=0) # ordenar colunas
# m_linha = numpy.sort(matriz, axis=1) # ordenar linhas
# m_col_lin = numpy.sort(m_linha, axis=1) #ordenar em colunas e linhas
...
# numpy.savetxt('vetor_a.txt', vetor_a, fmt='%f', delimiter=';') # nome do arquivo, tipo de dado, formato, delimitador dos elementos
